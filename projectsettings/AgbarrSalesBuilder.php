...
class AgbarrSalesBuilder extends BaseLcl {

    public function __construct($aid, $projectID, $gids) {
        $this->maintable = "agbarr_mults";        
        ...
        if(isset($_REQUEST['GID']) && $_REQUEST['GID'] == 3 ){
			// measure selection list
	        $this->pageArray["MEASURE_SELECTION_LIST"] = array(
	            array('measureID' => 0, 'jsonKey'=>'VALUE', 'measureName' => 'Value (£)', 'selected' => false ),
	            array('measureID' => 1, 'jsonKey'=>'VOLUME', 'measureName' => 'Volume', 'selected' => true)
	        );        	
        }        
        ...
        $this->uploadDir = __DIR__ . "/../../".$_REQUEST['projectDIR']."/templates/AGBARR/";
        $this->uploadUrl = $this->get_full_url()."/".$_REQUEST['projectDIR']."/templates/AGBARR/";
	...
	public function configureClassVars()
    {
        ...
        /* $this->link = " WHERE $this->maintable.accountID=$this->aid " .
                "AND $this->maintable.SNO=$this->storetable.SNO " .
                "AND $this->maintable.GID=$this->storetable.GID " .
                "AND $this->maintable.PIN=$this->skutable.PIN " .
                "AND $this->maintable.GID=$this->skutable.GID " .
                ...
                ((!empty($this->territorytable)) ? "AND $this->storetable.SNO=$this->territorytable.SNO AND $this->storetable.GID=$this->territorytable.GID AND $this->territorytable.GID IN ($this->GID) AND $this->territorytable.accountID = $this->aid " : "") .
                "AND $this->skutable.hide<>1 " .
                "AND $this->skutable.gid IN (".$this->GID.") ".
        ...
        $this->measureArray['M2']['VAL'] = "(SUM(IFNULL($this->ProjectValue,0))/SUM(IFNULL($this->ProjectVolume,1)))";
        $this->measureArray['M2']['ALIASE'] = "PRICE";
        $this->measureArray['M2']['dataDecimalPlaces'] = 2;

        $this->measureArray['M3']['VAL'] = "DISTINCT((CASE WHEN ".$this->ProjectValue." > 0 THEN ".$this->maintable.".SNO END))";
