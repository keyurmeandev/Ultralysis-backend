...
// year week
class FerreroMasterSystem extends BaseMults {
            ...
            if(!$this->hasMeasureFilter)
            {
                if (count($gids) == 1)
                {
                    $this->pageArray["MEASURE_SELECTION_LIST"] = array(
                        ...
                        array('measureID' => 3, 'jsonKey'=>'PRICE', 'measureName' => 'PRICE', 'selected' => true),
	...
	public function configureClassVars() {
        ...
        // JOIN (SELECT distinct mydate,gid FROM ferrero_mults ) b ON (period_list.mydate = b.mydate AND period_list.gid=b.gid)
        $this->timeHelperLink = " JOIN (SELECT distinct " . $this->maintable . "." . $this->dateperiod . "," . 
                                $this->maintable . ".GID FROM " . $this->maintable . " ) b ON ".
                                " (" . $this->timetable . ".mydate = b.mydate AND " . $this->timetable . ".gid = b.GID )".
