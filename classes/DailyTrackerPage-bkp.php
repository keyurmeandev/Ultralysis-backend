    ...
    *****/
    ...
    public function findMorrisonsItemStatus()
    {
        ...
        if ($this->settingVars->hasSeasonalTimeframe) {
            $requestedYear   = $this->settingVars->fromToDateRange['fromYear'];
            $requestedYearLy = $this->settingVars->fromToDateRange['lyToYear'];
            $requestedSeason = '';
        }else{
            $requestedCombination = explode("-", $_REQUEST['timeFrame']);
            $requestedYear        = $requestedCombination[1];
            $requestedYearLy      = $requestedYear - 1;  
            $requestedSeason      = $requestedCombination[0];
        }
        
        $status = false;
        $query = "SELECT ".$this->settingVars->pnameField." as PNAME FROM ".$this->settingVars->maintable." WHERE ".$this->settingVars->pnameField." NOT IN (SELECT DISTINCT ".$this->settingVars->pnameMorrisonsPriceTableField." FROM ".$this->settingVars->seasonalmorrisonspricetable." WHERE GID = ".$this->settingVars->morrisonsGid." ) AND GID = ".$this->settingVars->morrisonsGid." AND accountID = ".$this->settingVars->aid." AND seasonal_year = ".$requestedYear." GROUP BY PNAME"; // seasonal_description = '".$requestedSeason."' AND
        ...
        if(!$status)
        {
            $query = "SELECT DISTINCT ".$this->settingVars->pnameMorrisonsPriceTableField." FROM ".$this->settingVars->seasonalmorrisonspricetable.
                ...
                $this->settingVars->maintable.".PNAME=".$this->settingVars->seasonalmorrisonspricetable.".PNAME AND ".$this->settingVars->seasonalmorrisonspricetable.".GID = ".$this->settingVars->morrisonsGid." AND ".$this->settingVars->seasonalmorrisonspricetable.".PRICE = 0.01 AND ".$this->settingVars->maintable.".accountID = ".$this->settingVars->aid." AND ".$this->settingVars->maintable.".seasonal_year = ".$requestedYear; //AND ".$this->settingVars->maintable.".seasonal_description = '".$requestedSeason."'
    ...
    public function getColumnChartData() {
        ...
        $this->jsonOutput['lastYear'] = $ly = $requestedYearLy;
        ...
        if (is_array($this->settingVars->lyDates) && !empty($this->settingVars->lyDates)) {
            foreach ($this->settingVars->lyDates as $lyDate) {
                $colsHeader['LY'][] = array("FORMATED_DATE" => date('D-d-M', strtotime($lyDate)), "FORMATED_DATE_DAY" => date('D', strtotime($lyDate)) ,"MYDATE" => $lyDate);
            }
        }
    ...
    public function gridData() {
        ...
        $maintable = $this->settingVars->maintable;
        $this->jsonOutput['thisYear'] = $ty = (int) $requestedYear;
        $this->jsonOutput['lastYear'] = $ly = (int) $requestedYearLy;
    ...
    public function prepareExportData(){
        ...
        if ($this->settingVars->hasSeasonalTimeframe) {
            $requestedYear   = (int) $this->settingVars->fromToDateRange['fromYear'];
            $requestedYearLy = (int) $this->settingVars->fromToDateRange['lyToYear'];
            $requestedSeason = '';
        }else{
            $requestedCombination = explode("-", $_REQUEST['timeFrame']);
            $requestedYear        = $requestedCombination[1];
        ...
        $lastYear = $ly = (int) $requestedYearLy;
