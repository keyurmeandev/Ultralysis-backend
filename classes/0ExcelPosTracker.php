...
class ExcelPosTracker extends config\UlConfig {
    ...
    private function exportData() {
        ...
        echo '/usr/bin/perl '.dirname(__FILE__).'/../batch/ExcelPosTrackerReport.pl "'.$filePath.'" "'.$preparedQuery.'" "'.$timekey.'" "'.$timeval.'" "'.$measureFilterSettings.'" "'.$headerColumns.'" "'.$imgLogoPath.'"';
        exit;
        $result = shell_exec('/usr/bin/perl '.dirname(__FILE__).'/../batch/ExcelPosTrackerReport.pl "'.$filePath.'" "'.$preparedQuery.'" "'.$timekey.'" "'.$timeval.'" "'.$measureFilterSettings.'" "'.$imgLogoPath.'"');
        $this->jsonOutput['downloadLink'] = $this->settingVars->get_full_url()."/".basename(dirname(dirname(__FILE__)))."/uploads/Excel-Pos-Tracker/".$fileName;
    }
