...
class BottomOuts extends config\UlConfig {
    /*     * ***
	 * go()
     ...
     * @return $this->jsonOutput with all data that should be sent to client app     
     */
    private function bottomOutsGrid() {
		$bottomOutsGridDataBinding = array();
		...
		if (is_array($result) && !empty($result)) {
			...
			$query = "SELECT "
				...
				"GROUP BY TPNB,SNO ".$addTerritoryGroup.",MYDATE";
			// echo $query; exit;
			$result = $this->queryVars->queryHandler->runQuery($query, $this->queryVars->linkid, db\ResultTypes::$TYPE_OBJECT);
			...
			foreach($result as $value){
				$territoryIndex = $addTerritoryColumn=='' ? '' : '_'.$value['TERRITORY'];
				$index = $value['TPNB'].'_'.$value['SNO'].$territoryIndex;
				$zeroSaleDays[$index] = isset($zeroSaleDays[$index]) ? $zeroSaleDays[$index] : 0;
				$zeroSaleDays[$index] += 1; 
			}
			...
			$query = "SELECT "
					...
					"HAVING SALES > 0 ORDER BY SALES DESC";
			// echo $query; exit;
			...
			if (is_array($result) && !empty($result)) {
				...
				foreach ($result as $value) {
					...
					$territoryIndex = $addTerritoryColumn=='' ? '' : '_'.$value['TERRITORY'];
					$zeroSaleDaysIndex 	= $value['TPNB'] . '_' . $value['SNO'].$territoryIndex;
					// where zero sale days > 0
					if( array_key_exists($zeroSaleDaysIndex, $zeroSaleDays) )
					{
						...
						if (($_REQUEST['VSI'] == 1 && $status == 1) || ($_REQUEST['VSI'] == 2 && $status == 0) || ($_REQUEST['VSI'] == 3)) {
							...
							$row['SALES_QTY'] 		= (int)$qtyOfSales;
							$row['ZERO_SALE_DAYS'] 	= $zeroSaleDays[$zeroSaleDaysIndex];
							...
							array_push($bottomOutsGridDataBinding, $row);				
						}
				...
				foreach ($bottomOutsGridDataBinding as $key => $value) {
					//still going to sort by firstname
					$emp[$key] = $value['SALES_QTY'];
				}

                if (is_array($emp) && !empty($emp))
				    array_multisort($emp, SORT_DESC, $bottomOutsGridDataBinding);
			
			}
		
		}
		
        $this->jsonOutput['bottomOutsGrid'] = $bottomOutsGridDataBinding;
    }
