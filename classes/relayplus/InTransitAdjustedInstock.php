...
class InTransitAdjustedInstock extends config\UlConfig {
    /*     * ***
     * go()
    ...
    public function go($settingVars) {
        ...
        switch ($action) {
            ...
            case 'filterBySku':
                $this->outofstockGrid();
                $this->showNotValidStock();
                break;
            case 'inTransitAdjustedInstockGrid':
                $this->inTransitAdjustedInstockGrid();
                break;
    ...
    private function downloadExcelFile()
    {
        ...
        $fileName = "In-Transit-Adjusted-Instock-StoreListDL-" . date("Y-m-d-h-i-s") . ".xlsx";
        $savePath = dirname(__FILE__)."/../../uploads/In-Transit-Adjusted-Instock/";
        chdir($savePath);
        $objWriter->save($fileName);
        $this->jsonOutput['downloadLink'] = $this->settingVars->get_full_url()."/".basename(dirname(dirname(dirname(__FILE__))))."/uploads/In-Transit-Adjusted-Instock/".$fileName;
    }
    
    /**
     * inTransitAdjustedInstockGrid()
     * This Function is used to retrieve list based on set parameters     
     * @access private  
     * @return $this->jsonOutput with all data that should be sent to client app     
     */
    private function inTransitAdjustedInstockGrid() {
        ...
        /* **************************** */ 
        $subQueryToCollectStoresHavingValidOHQLess = "(SELECT SIN AS SIN".
                    ",SNO AS SNO".
                    ",SUM(OHQ) AS SumOfOHQ ". 
        ...
        $query  = "SELECT a.SIN SKUID".
                    ",COUNT(*) AS ADJUSTED_INSTOCK_STORES ".
                    "FROM $subQueryToCollectStoresHavingValidOHQLess a INNER JOIN ".$this->settingVars->ranged_items." ".
                    "ON (a.SIN = ".$this->settingVars->ranged_items.".skuID AND a.SNO = ".$this->settingVars->ranged_items.".SNO) ".$extraTable.
                    " WHERE ".$this->settingVars->ranged_items.".VSI = 1 AND ".$this->settingVars->ranged_items.".StoreTrans > 0 AND ".$this->settingVars->ranged_items.".clientID='".$this->settingVars->clientID."' $territoryPart GROUP BY SKUID ".
                    "ORDER BY SKUID";
        //echo $query; exit;
        $redisOutput = $this->redisCache->checkAndReadByQueryHashFromCache($query);
        if ($redisOutput === false) {
        ...
        //$adjustedInstockStoreCounterArray = array();
        
        if (is_array($result) && !empty($result)) {
            foreach ($result as $key => $data) {
                $storeCounterArray[$data['SKUID']]['VALID_STORES_ADJUSTED_INSTOCK'] = $data['ADJUSTED_INSTOCK_STORES'];
            }
        }
        ...
        if ((is_array($result) && !empty($result)) && (is_array($storeCounterArray) && !empty($storeCounterArray))) {
            ...
            foreach ($result as $key => $value) {
                ...
                $adjustedInstockStores   = array_key_exists($value['SKUID'], $storeCounterArray) ? (int)$storeCounterArray[$index]['VALID_STORES_ADJUSTED_INSTOCK'] : 0;
                ...
                $dataSet[$cnt]['ADJUSTED_INSTOCK']  = $validStores == 0 ? 0 : (($instockStores+$adjustedInstockStores)/$validStores)*100;
        ...
        $this->jsonOutput['inTransitAdjustedInstockGrid'] = $dataSet;
    }
    ...
    public function showNotValidStock() {

        $storePart = "";
        if(isset($_REQUEST["FS"]["F2"]) && $_REQUEST["FS"]["F2"] != "" ) {
            $storePart = " AND SIN = '".$_REQUEST["FS"]["F2"]."' ";
        }
        ...
        $subQueryToCollectStoresHavingValidOHQ = "(SELECT SIN AS SIN".
                    ...
                    "FROM ".$this->settingVars->maintable." ". 
                    "WHERE ".$this->settingVars->DatePeriod."= '" . filters\timeFilter::$tyDaysRange[0] . "' ".
                    "AND OpenDate < insertdate $storePart ".
                    "GROUP BY SIN,SNO ".
                    "HAVING SumOfOHQ>0)";
        ...
        if(isset($_REQUEST["FS"]["F2"]) && $_REQUEST["FS"]["F2"] != "" ) {
            $storePart = " AND a.SIN = '".$_REQUEST["FS"]["F2"]."' ";
        }

        $query  = "SELECT a.SIN SKUID".
                    ",a.SNO AS INVALID_WSTOCK_STORES ".
                    " FROM $subQueryToCollectStoresHavingValidOHQ a LEFT OUTER JOIN ".$this->settingVars->ranged_items." b ".
                    "ON (a.SIN = b.skuID AND a.SNO = b.SNO) $joinPart 
                    WHERE b.skuID IS NULL OR b.SNO IS NULL AND b.clientID='".$this->settingVars->clientID."' AND b.GID = ".$this->settingVars->GID."  $territoryPart $storePart GROUP BY SKUID, INVALID_WSTOCK_STORES ".
                    "ORDER BY SKUID";
        //echo $query; exit;
        $redisOutput = $this->redisCache->checkAndReadByQueryHashFromCache($query);
        if ($redisOutput === false) {
        ...
        if (is_array($result) && !empty($result)) {
            foreach ($result as $key => $data){
                $storeCounterArray[] = $data['INVALID_WSTOCK_STORES'];
            }
            $storeArray = implode("','",$storeCounterArray);
        }


        //echo "<pre>";print_r($storeCounterArray);exit;
        
        /** 
        * END COLLECTING NOT-VALID STORES HAVING STOCK [NOT VALID W/STOCK] *
        **/
        ...
        $tsiStatusData = $vsiStatusData = $storeTransData = array();
        $withstockGridDataBinding = array();
        if (is_array($result) && !empty($result)) {
            ...
            if (is_array($result) && !empty($result)) {
                ...
                foreach ($result as $value) {
                    ...
                    //if( $status == 1 ) {
                        ...
                        $row['SKU']             = utf8_encode($value['SKU']);
                        $row['STOCK']           = $value['STOCK'];
                        $row['TRANSIT']         = ($storeTransData[$index] == null) ? 0 : $storeTransData[$index];
                        $row['SHELF']           = $value['SHELF'];
                        $row['AVE_DAILY_SALES'] = ($aveDailySales > 0) ? number_format($aveDailySales, 2, '.', '') : 0;
                        $row['LAST_SALE']       = ($lastSalesDays[$row['SKUID'] . "_" . $row['SNO']] > 0) ? $lastSalesDays[$row['SKUID'] . "_" . $row['SNO']] : 0;
                        $row['PLANOGRAM']       = ($value['PLANOGRAM'] == null) ? '' : $value['PLANOGRAM'];
                        $row['TSIVSI']          = $tsiVsi;
                        ...
                        array_push($withstockGridDataBinding, $row);
                   //}
                }

                foreach ($withstockGridDataBinding as $key => $value) {
                    //still going to sort by firstname
                    $emp[$key] = $value['AVE_DAILY_SALES'];
                }
                if (is_array($emp) && !empty($emp))
                    array_multisort($emp, SORT_DESC, $withstockGridDataBinding);
            }
        }

        $this->jsonOutput['noStockGrid'] = $withstockGridDataBinding;
