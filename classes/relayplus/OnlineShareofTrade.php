...
class OnlineShareofTrade extends config\UlConfig {
    /*     * ***
     * go()
    ...
    public function go($settingVars) {
        ...
        switch ($action) {
            case 'onlineShareByItemGrid':
                $this->onlineShareByItemGrid();
                break;
            case 'onlineShareByItemChart':
                $this->onlineShareByItemChart();
                break;
            case 'getPinData':
                $this->getPinData();
                break;
    ...
    private function getPinData()
    {
        ...
        $this->settingVars->measureTypeField." = 'ONLINE_DELIVERED_VOLUME' AND ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).") AND ".$this->ferreroAsdaOnlineTable.".pin = ".$_REQUEST['clickedPin']." GROUP BY PERIOD ORDER BY PERIOD";
        ...
        $onlineDeliveredData = $percentageColumnChartData = array();
        if (is_array($result) && !empty($result)) 
        {
            foreach($result as $key => $data)
            {
                ...
                $onlineDeliveredData[] = (double)$data['ONLINE_DELIVERED_YTD'];
                $percentageColumnChartData[] = ($result[$key]['VOLUME_ALL_YTD'] > 0) ? ( ($data['ONLINE_DELIVERED_YTD']/$result[$key]['VOLUME_ALL_YTD'] )*100) : 0;
            }
        }
        
        $this->jsonOutput['onlineDeliveredData'] = $onlineDeliveredData;
        $this->jsonOutput['percentageColumnChartData'] = $percentageColumnChartData;
    }
    
    /**
     * onlineShareByItemGrid()
     * This Function is used to retrieve list based on set parameters     
     * @access private  
     * @return $this->jsonOutput with all data that should be sent to client app     
     */
    private function onlineShareByItemGrid() {
        ...
        $query = "SELECT DISTINCT ".$this->pinField." as PIN, ".$this->pnameField." as PNAME, ".$this->ferreroAsdaOnlineTable.".dept_name as DEPARTMENT, ".
        "SUM((CASE WHEN ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).") AND measure_type = 'ONLINE_DELIVERED_VOLUME' THEN 1 ELSE 0 END) * ".$this->ferreroAsdaOnlineTable.".value) as ONLINE_DELIVERED_YTD, ".
        ...
        "FROM ".$this->ferreroAsdaOnlineTable.", ".$this->skutable." WHERE ".$this->ferreroAsdaOnlineTable.".pin = ".$this->pinField." AND ".$this->ferreroAsdaOnlineTable.".gid = ".$this->skutable.".gid AND ".$this->ferreroAsdaOnlineTable.".gid = ".$this->settingVars->GID." AND ".$this->skutable.".gid = ".$this->settingVars->GID." AND ".$this->skutable.".clientID = '".$this->settingVars->clientID."' AND ".
        $this->settingVars->measureTypeField." = 'ONLINE_DELIVERED_VOLUME' AND ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).")";
        ...
        if (is_array($result) && !empty($result)) {
            ...
            foreach($result as $key => $data)
            {
                $result[$key]['VOLUME_ALL_YTD'] = (double)$summaryPinQtySum[$data['PIN']]['VOLUME_ALL_YTD'];
                $result[$key]['VOLUME_ALL_L13'] = (double)$summaryPinQtySum[$data['PIN']]['VOLUME_ALL_L13'];
                $result[$key]['VOLUME_ALL_L4'] = (double)$summaryPinQtySum[$data['PIN']]['VOLUME_ALL_L4'];
                
                $result[$key]['SHARE_ONLINE_YTD'] = ($result[$key]['VOLUME_ALL_YTD'] > 0) ? (($data['ONLINE_DELIVERED_YTD']*100)/$result[$key]['VOLUME_ALL_YTD']) : 0;
                $result[$key]['SHARE_ONLINE_L13'] = ($result[$key]['VOLUME_ALL_L13'] > 0) ? (($data['ONLINE_DELIVERED_L13']*100)/$result[$key]['VOLUME_ALL_L13']) : 0;
                $result[$key]['SHARE_ONLINE_L4'] = ($result[$key]['VOLUME_ALL_L4'] > 0) ? (($data['ONLINE_DELIVERED_L4']*100)/$result[$key]['VOLUME_ALL_L4']) : 0;
            }
        }
    ...
    private function onlineShareByItemChart() {

        if(!isset($this->ytdAsdaPeriodList)){
            $query = "SELECT DISTINCT period as PERIOD FROM ".$this->ferreroAsdaOnlineTable." WHERE ".$this->ferreroAsdaOnlineTable.".gid = ".$this->settingVars->GID." AND ".$this->ferreroAsdaOnlineTable.".accountID = ".$this->settingVars->aid." ORDER BY PERIOD DESC";
            $redisOutput = $this->redisCache->checkAndReadByQueryHashFromCache($query);
            if ($redisOutput === false) {
                $asdaPeriodList = $this->queryVars->queryHandler->runQuery($query, $this->queryVars->linkid, db\ResultTypes::$TYPE_OBJECT);
        ...
        $qryPart = '';
        if (isset($_REQUEST['clickedPin']) && !empty($_REQUEST['clickedPin']))
            $qryPart = " AND ".$this->ferreroAsdaOnlineTable.".pin = ".$_REQUEST['clickedPin'];

        $query = "SELECT ".$this->ferreroAsdaOnlineTable.".period, SUM((CASE WHEN ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).") AND measure_type = 'ONLINE_DELIVERED_VOLUME' THEN 1 ELSE 0 END) * ".$this->ferreroAsdaOnlineTable.".value) as ONLINE_DELIVERED, ".
            "SUM((CASE WHEN ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).") AND measure_type = 'ONLINE_VALUE' THEN 1 ELSE 0 END) * ".$this->ferreroAsdaOnlineTable.".value) as ONLINE_VALUE ".
            "FROM ".$this->ferreroAsdaOnlineTable.", ".$this->skutable." WHERE ".$this->ferreroAsdaOnlineTable.".pin = ".$this->pinField." AND ".$this->ferreroAsdaOnlineTable.".gid = ".$this->skutable.".gid AND ".$this->ferreroAsdaOnlineTable.".gid = ".$this->settingVars->GID." AND ".$this->skutable.".gid = ".$this->settingVars->GID." AND ".$this->skutable.".clientID = '".$this->settingVars->clientID."' AND ".$this->settingVars->measureTypeField." IN ('ONLINE_VALUE','ONLINE_DELIVERED_VOLUME') AND ".$this->ferreroAsdaOnlineTable.".period IN (". implode(",",$this->ytdAsdaPeriodList).") ".$qryPart." GROUP BY ".$this->ferreroAsdaOnlineTable.".period ";
        ...
        if (is_array($result) && !empty($result)) {
            foreach ($result as $k1 => $v1) {
                $result[$k1]['ONLINE_DELIVERED'] = (float) $v1['ONLINE_DELIVERED'];
                $result[$k1]['ONLINE_VALUE']     = (float) $v1['ONLINE_VALUE'];
                $result[$k1]['PRICE']            = (float) ($v1['ONLINE_DELIVERED'] > 0) ? $v1['ONLINE_VALUE'] / $v1['ONLINE_DELIVERED'] : 0;
            }
        }
        array_multisort(array_column($result, 'period'), SORT_ASC, $result);
        $this->jsonOutput['lineChartSeriesData'] = $result;
    }
}
?>