...
class ContributionAnalysis extends config\UlConfig {
    ...
    public function configurePage() {
        ...
        $this->accountName = (!$this->skipDbcolumnArray) ? strtoupper($this->dbColumnsArray[$getField]) : $getField;
    }
    ...
    public function buildPageArray() {
     ...
     public function GridAll() {
        ...
        $this->accountName = strtoupper($this->accountName);
        //$id = !key_exists('ID', $this->settingVars->dataArray[$this->accountName]) ? $this->settingVars->dataArray[$this->accountName]["NAME"] : $this->settingVars->dataArray[$this->accountName]["ID"];
        ...
        if(!empty($ExtraCols) && count($ExtraCols)>0)
            $requiredGridFields = array_merge(array("ACCOUNT", $havingTYValue, $havingLYValue),array_column($ExtraCols,'NAME_ALIASE'));
        else
        ...
        foreach ($gridNames as $gridKey => $currentGrid) { //TRAVERSING ALL GRID NAMES ['NPD' , .......]
            ...
            foreach ($currentGrid as $data) {
                ...
                $temp['VAR'] = $data['VAR'];
                $temp['PERCENT'] = number_format(($data['VAR'] / $totals[$gridKey]) * 100, 1, '.', '');
    ...
    private function skuSelect() {
        ...
        if(isset($_REQUEST['ACCOUNT']) && !empty(urldecode($_REQUEST['ACCOUNT'])))
            $this->queryPart .=" AND $name = '".trim(urldecode($_REQUEST['ACCOUNT']))."' ";
