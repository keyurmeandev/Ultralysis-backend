...
class InitialiseProject extends config\UlConfig {
    ...
    public function go($settingVars) {
        ...
        switch ($this->projectType)
        {
            case 'tesco-store-daily':
                ...
                else
                {
                    ...
                    $this->jsonOutput['isSifPageRequired'] = $this->settingVars->isSifPageRequired;    
            ...
            case 'relayplus':
                ...
                else
                {
                    ...
                    if(isset($this->settingVars->weekEndingText) && !empty($this->settingVars->weekEndingText))
                        $this->jsonOutput['weekEndingText'] = $this->settingVars->weekEndingText;                        
        ...
        $this->jsonOutput['isShowCustomerLogo'] = true;
        ...
        if(isset($this->queryVars->projectConfiguration) && isset($this->queryVars->projectConfiguration['has_product_market_filter_disp_type']) && $this->queryVars->projectConfiguration['has_product_market_filter_disp_type']==1){
            $this->getAllProductAndMarketInlineFilterData($this->queryVars, $this->settingVars);
        }
        /*[END] PREPARING THE PRODUCT INLINE FILTER DATA */

        /*[START] SETUP THE FREE SCOPE VARS */
        if(isset($this->settingVars->fetchProductAndMarketFilterOnTabClick) && $this->settingVars->fetchProductAndMarketFilterOnTabClick == true) {
            $this->jsonOutput['fetchProductAndMarketFilterOnTabClick'] = $this->settingVars->fetchProductAndMarketFilterOnTabClick;
        }
        /*[END] SETUP THE FREE SCOPE VARS */

        /*[START] TREEMAP Color code*/
                $this->jsonOutput['positiveStartColorCode'] = $this->settingVars->positiveStartColorCode;
                $this->jsonOutput['positiveEndColorCode']   = $this->settingVars->positiveEndColorCode;
                $this->jsonOutput['negativeStartColorCode'] = $this->settingVars->negativeStartColorCode;
                $this->jsonOutput['negativeEndColorCode']   = $this->settingVars->negativeEndColorCode;
                $this->jsonOutput['newItemColorCode']       = $this->settingVars->newItemColorCode;
    ...
    public function getAllSkus()
    {
        ...
        if($this->projectType == "tesco-store-daily" || $this->projectType == "impulseViewJS")
        {
            ...
            unset($allSkus[0]);
            $this->jsonOutput['skuListRetailLink'] = array_values($allSkus);
        }
    }
    ...
    function getLatestDateOfMainProject() {
        ...
        $this->jsonOutput['tsdMyStoreBasedate'] = $result;
    }

    function getLatestDateOfDcTable() {
        $has_dc_stock_inc = $this->queryVars->projectConfiguration['has_dc_stock_inc'];
        if (isset($has_dc_stock_inc) && $has_dc_stock_inc == 0) {
        ...
        if (!empty($dataHelpers)) {
            foreach ($dataHelpers as $key => $account) {
                if($account != "")
                {
                    ...
                    if(in_array($this->settingVars->dataArray[$combineAccounts[0]]['TYPE'],['A','M','T']))
                    {
