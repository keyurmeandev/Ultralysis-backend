    ...
    *****/
    public function go($settingVars) {
		...
		if (isset($_REQUEST['action']) && $_REQUEST['action']=='export') {
            ...
            /*echo '/usr/bin/perl '.dirname(__FILE__).'/../batch/DailyTrackerReport.pl "'.$filePath.'" "'.$preparedQuery.'" "'.$thisYear.'" "'.$lastYear.'" "'.$actXlsFldNm.'" "'.$actXlsValNm.'" "'.$havingTYValue.'" "'.$havingLYValue.'" "'.$appliedFiltersTxt.'" "'.$imgLogoPath.'" "'.$tyFromDate.'" "'.$tyToDate.'"';
            exit;*/
        ...
        foreach (array_keys($seasonalDataArray) as $account) {
            ...
            foreach ($dateArray as $dayMydate => $dayMonth) {
                ...
                if (isset($seasonalDataArray[$account][$dayMonth])) {
                    ...
                    $tmp[$dtKey] = round($data[$havingTYValue]);
                    $tmp['YEAR'] = $ty;
                    $tmp['RANK'] = (int)'1'.$data['YEAR'];
                    $tmp['ROWDESC'] = 'Daily Sales';
                    $tmp['highlightRow'] = 1;

                    $cmpTyTotal[$dtKey] += round($data[$havingTYValue]);
                    
                    $tmp1[$dtKey] = $data[$havingLYValue];
                    $tmp1['YEAR'] = $ly;
                    $tmp1['RANK'] = (int)'1'.($data['YEAR']-1);
                    $tmp1['ROWDESC'] = 'Daily Sales';
                    $tmp1['highlightRow'] = 1;
                    
                    $cmpLyTotal[$dtKey] += round($data[$havingLYValue]);
