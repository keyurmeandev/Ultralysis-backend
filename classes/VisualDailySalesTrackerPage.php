...
class VisualDailySalesTrackerPage extends config\UlConfig {
    ...
    *****/
    ...
    public function gridData() {
        ...
        */
        ...
        /*[START] COLOR CODE CHANGE LOGIC */
            $negativeEndColorCode   = '#500000';
            $negativeStartColorCode = '#f00000';
            $newItemColorCode       = '#FFFF99';
            $positiveEndColorCode   = '#007A00'; //#005000
            $positiveStartColorCode = '#00f000';

            $positiveColorCode      = $positiveStartColorCode;
            $positiveStartColorCode = $this->hexToRgb($positiveStartColorCode);
            $positiveEndColorCode   = $this->hexToRgb($positiveEndColorCode);
            $negativeColorCode      = $negativeStartColorCode;
            $negativeStartColorCode = $this->hexToRgb($negativeStartColorCode);
            $negativeEndColorCode   = $this->hexToRgb($negativeEndColorCode);
        /*[END] COLOR CODE CHANGE LOGIC */

        $cnt = 0; $cmpTyTotal = $cmpLyTotal = []; $allYoyData = [];
        ...
        foreach (array_keys($seasonalDataArray) as $account) {
            ...
            /*[START] COLOR CODE CHANGE LOGIC */
                /*$positiveMaxTyVal = $positiveMinTyVal = $negativeMaxTyVal = $negativeMinTyVal = 0;
                $allTyData = array_column($seasonalDataArray[$account],$havingTYValue);
                $allPositiveTyVals = array_filter($allTyData, function ($v){ return $v > 0; });
                if (!empty($allPositiveTyVals)) {
                    $positiveMaxTyVal = max($allPositiveTyVals);
                    $positiveMaxTyVal = round($positiveMaxTyVal);
                    $positiveMinTyVal = min($allPositiveTyVals);
                    $positiveMinTyVal = round($positiveMinTyVal);
                }
                $allNegativeTyVals = array_filter($allTyData, function ($v){ return $v < 0; });
                if (!empty($allNegativeTyVals)) {
                    $negativeMaxTyVal = max($allNegativeTyVals);
                    $negativeMaxTyVal = round($negativeMaxTyVal);
                    $negativeMinTyVal = min($allNegativeTyVals);
                    $negativeMinTyVal = round($negativeMinTyVal);
                }
                
                $positiveMaxLyVal = $positiveMinLyVal = $negativeMaxLyVal = $negativeMinLyVal = 0;
                $allLyData = array_column($seasonalDataArray[$account],$havingLYValue);
                $allPositiveLyVals = array_filter($allLyData, function ($v){ return $v > 0; });
                if (!empty($allPositiveLyVals)) {
                    $positiveMaxLyVal = max($allPositiveLyVals);
                    $positiveMaxLyVal = round($positiveMaxLyVal);
                    $positiveMinLyVal = min($allPositiveLyVals);
                    $positiveMinLyVal = round($positiveMinLyVal);
                }

                $allNegativeLyVals = array_filter($allLyData, function ($v){ return $v < 0; });
                if (!empty($allNegativeLyVals)) {
                    $negativeMaxLyVal = max($allNegativeLyVals);
                    $negativeMaxLyVal = round($negativeMaxLyVal);
                    $negativeMinLyVal = min($allNegativeLyVals);
                    $negativeMinLyVal = round($negativeMinLyVal);
                }*/
            /*[END] COLOR CODE CHANGE LOGIC */
            ...
            foreach ($dateArray as $dayMydate => $dayMonth) {
                ...
                if (isset($seasonalDataArray[$account][$dayMonth])) {
                    ...
                    $tmp['highlightRow'] = 1;
                    $colorCodeTy = $this->renderColorCode($positiveMaxTyVal, $positiveMinTyVal, $negativeMaxTyVal, $negativeMinTyVal, $tmp[$dtKey], $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);
                    $tmp[$dtKey.'highlightRowColorCode'] = $colorCodeTy;*/
                    
                    $cmpTyTotal[$dtTotalKey] += $data[$havingTYValue];

                    // $res[$dtTotalKey] += $data[$havingTYValue];;
                   ...
                   /* $tmp1[$dtKey] = $data[$havingLYValue]*1;
                    ...
                    $tmp1['highlightRow'] = 1;
                    $colorCodeLy = $this->renderColorCode($positiveMaxLyVal, $positiveMinLyVal, $negativeMaxLyVal, $negativeMinLyVal, $tmp1[$dtKey], $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);
                    $tmp1[$dtKey.'highlightRowColorCode'] = $colorCodeLy;*/
                    ...
                    $allYoyData[$account][$dtKey] = $cumTmp2[$dtKey];
            ...
            $allPositiveYoyVals = array_filter($allYoyData[$account], function ($v){ return $v > 0; });
            if (!empty($allPositiveYoyVals)) {
                $positiveMaxYoyVal = max($allPositiveYoyVals);
                $positiveMaxYoyVal = round($positiveMaxYoyVal);
                $positiveMinYoyVal = min($allPositiveYoyVals);
                $positiveMinYoyVal = round($positiveMinYoyVal);
            }
            $allNegativeYoyVals = array_filter($allYoyData[$account], function ($v){ return $v < 0; });
            if (!empty($allNegativeYoyVals)) {
                $negativeMaxYoyVal = max($allNegativeYoyVals);
                $negativeMaxYoyVal = round($negativeMaxYoyVal);
                $negativeMinYoyVal = min($allNegativeYoyVals);
                $negativeMinYoyVal = round($negativeMinYoyVal);
            }

            foreach ($cumTmp2 as $keyDt => $valDt) {
                $colorCodeYoy = $this->renderColorCode($positiveMaxYoyVal, $positiveMinYoyVal, $negativeMaxYoyVal, $negativeMinYoyVal, $valDt, $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);

                $cumTmp2[$keyDt.'highlightRowColorCode'] = $colorCodeYoy;
            }
        ...
        $cumTotalLYData = $cumTotalTYData = $cmpTyLyYoyCumTotal = [];
        $tySum = $lySum = 0;
        ...
        foreach ($dateArray as $dayMydate => $dayMonth) {
            ...
            $tySum += $cmpTyTotal[$dtKey];
            $cumTotalTYData[$dtKey] = $tySum;

            $lySum += $cmpLyTotal[$dtKey];
            $cumTotalLYData[$dtKey] = $lySum;

            $cmpTyLyYoyCumTotal[$dtKey] = ($cumTotalLYData[$dtKey] > 0) ? ((($cumTotalTYData[$dtKey] - $cumTotalLYData[$dtKey]) / $cumTotalLYData[$dtKey])*100) : 0;
                
        }

        $allPositiveTyLyYoyValsData = array_filter($cmpTyLyYoyCumTotal, function ($v){ return $v > 0; });
        if (!empty($allPositiveTyLyYoyValsData)) {
            $positiveTyLyMaxYoyVal = max($allPositiveTyLyYoyValsData);
            $positiveTyLyMaxYoyVal = round($positiveTyLyMaxYoyVal);
            $positiveTyLyMinYoyVal = min($allPositiveTyLyYoyValsData);
            $positiveTyLyMinYoyVal = round($positiveTyLyMinYoyVal);
        }
        $allNegativeTyLyYoyVals = array_filter($cmpTyLyYoyCumTotal, function ($v){ return $v < 0; });
        if (!empty($allNegativeTyLyYoyVals)) {
            $negativeTyLyMaxYoyVal = max($allNegativeTyLyYoyVals);
            $negativeTyLyMaxYoyVal = round($negativeTyLyMaxYoyVal);
            $negativeTyLyMinYoyVal = min($allNegativeTyLyYoyVals);
            $negativeTyLyMinYoyVal = round($negativeTyLyMinYoyVal);
        }

        foreach ($cmpTyLyYoyCumTotal as $keyDt => $valDt) {
            $colorCodeTyLyYoy = $this->renderColorCode($positiveTyLyMaxYoyVal, $positiveTyLyMinYoyVal, $negativeTyLyMaxYoyVal, $negativeTyLyMinYoyVal, $valDt, $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);
             $cmpTyLyYoyCumTotal[$keyDt.'highlightRowColorCode'] = $colorCodeTyLyYoy;
        }
        $finalData[] = $cmpTyLyYoyCumTotal;

        /*[START] ADDING THE TOTAL COLUMN*/
        $cmpTyLyYoyTotal = $cmpTyCumTotal = $cmpLyCumTotal = [];
        ...
        //     /*[START] COLOR CODE CHANGE LOGIC */
        //         $positiveMaxTotalTyVal = $positiveMinTotalTyVal = $negativeMaxTotalTyVal = $negativeMinTotalTyVal = 0;
        //         $allPositiveTotalTyVals = array_filter($cmpTyTotal, function ($v){ return $v > 0; });
        //         if (!empty($allPositiveTotalTyVals)) {
        //             $positiveMaxTotalTyVal = max($allPositiveTotalTyVals);
        //             $positiveMaxTotalTyVal = round($positiveMaxTotalTyVal);
        //             $positiveMinTotalTyVal = min($allPositiveTotalTyVals);
        //             $positiveMinTotalTyVal = round($positiveMinTotalTyVal);
        //         }

        //         $allNegativeTotalTyVals = array_filter($cmpTyTotal, function ($v){ return $v < 0; });
        //         if (!empty($allNegativeTotalTyVals)) {
        //             $negativeMaxTotalTyVal = max($allNegativeTotalTyVals);
        //             $negativeMaxTotalTyVal = round($negativeMaxTotalTyVal);
        //             $negativeMinTotalTyVal = min($allNegativeTotalTyVals);
        //             $negativeMinTotalTyVal = round($negativeMinTotalTyVal);
        //         }

        //         $positiveMaxTotalLyVal = $positiveMinTotalLyVal = $negativeMaxTotalLyVal = $negativeMinTotalLyVal = 0;
        //         $allPositiveTotalLyVals = array_filter($cmpLyTotal, function ($v){ return $v > 0; });
        //         if (!empty($allPositiveTotalLyVals)) {
        //             $positiveMaxTotalLyVal = max($allPositiveTotalLyVals);
        //             $positiveMaxTotalLyVal = round($positiveMaxTotalLyVal);
        //             $positiveMinTotalLyVal = min($allPositiveTotalLyVals);
        //             $positiveMinTotalLyVal = round($positiveMinTotalLyVal);
        //         }

        //         $allNegativeTotalLyVals = array_filter($cmpLyTotal, function ($v){ return $v < 0; });
        //         if (!empty($allNegativeTotalLyVals)) {
        //             $negativeMaxTotalLyVal = max($allNegativeTotalLyVals);
        //             $negativeMaxTotalLyVal = round($negativeMaxTotalLyVal);
        //             $negativeMinTotalLyVal = min($allNegativeTotalLyVals);
        //             $negativeMinTotalLyVal = round($negativeMinTotalLyVal);
        //         }
        //     /*[END] COLOR CODE CHANGE LOGIC */
        ...
        //         /*[START] COLOR CODE CHANGE LOGIC */
        //             $colorCodeTy = $this->renderColorCode($positiveMaxTotalTyVal, $positiveMinTotalTyVal, $negativeMaxTotalTyVal, $negativeMinTotalTyVal, $cmpTyTotal[$cpTykey], $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);
        //             $cmpTyTotal[$cpTykey.'highlightRowColorCode'] = $colorCodeTy;
        //         /*[END] COLOR CODE CHANGE LOGIC */
        ...
        //             /*[START] COLOR CODE CHANGE LOGIC */
        //                 $colorCodeLy = $this->renderColorCode($positiveMaxTotalLyVal, $positiveMinTotalLyVal, $negativeMaxTotalLyVal, $negativeMinTotalLyVal, $cmpLyTotal[$cpTykey], $positiveStartColorCode, $positiveEndColorCode, $negativeStartColorCode, $negativeEndColorCode, $newItemColorCode);
        //                 $cmpLyTotal[$cpTykey.'highlightRowColorCode'] = $colorCodeLy;
        //             /*[END] COLOR CODE CHANGE LOGIC */
    ...
    /*[START] DYNAMIC COLORCODE GENERATION LOGIC*/
        public function calculateColorCode($val, $rgbStartPoint, $rgbEndPoint) {
            if($val < 0) $val = $val*-1;
            $cdR = round((float)(($val * ($rgbStartPoint['r'] - $rgbEndPoint['r'])) + $rgbEndPoint['r']));
            $cdG = round((float)(($val * ($rgbStartPoint['g'] - $rgbEndPoint['g'])) + $rgbEndPoint['g']));
            $cdB = round((float)(($val * ($rgbStartPoint['b'] - $rgbEndPoint['b'])) + $rgbEndPoint['b']));
            return $this->fromRGBtoHex($cdR,$cdG,$cdB);
        }

        public function calculateValPerRetio($val, $minVal, $maxVal) {
            if($val < 0) $val = $val*-1;
            $pVAL = (($maxVal - $minVal) > 0 && ($val - $minVal) >= 0) ? (float) (($val - $minVal) / ($maxVal - $minVal)) : 0;
            return $pVAL;
        }

        public function renderColorCode($positiveMaxVal, $positiveMinVal, $negativeMaxVal, $negativeMinVal, $val, $rgbPositiveStartPoint, $rgbPositiveEndPoint, $rgbNegativeStartPoint, $rgbNegativeEndPoint, $newItemColorCode) {
            $colorCode = '';
            if ($val == 0) {
                $colorCode = $newItemColorCode;
            } else if ($val > 0) {
                $val = round($val);
                $pVal = $this->calculateValPerRetio($val, $positiveMinVal, $positiveMaxVal);
                if ($val == $positiveMaxVal) {
                    $colorCode = $this->fromRGBtoHex($rgbPositiveStartPoint['r'],$rgbPositiveStartPoint['g'],$rgbPositiveStartPoint['b']);
                }else if ($val == $positiveMinVal) {
                    $colorCode = $this->fromRGBtoHex($rgbPositiveEndPoint['r'],$rgbPositiveEndPoint['g'],$rgbPositiveEndPoint['b']);
                }else if ($pVal == 0) {
                    $colorCode = $this->fromRGBtoHex($rgbPositiveStartPoint['r'],$rgbPositiveStartPoint['g'],$rgbPositiveStartPoint['b']);
                }else{
                    $colorCode = $this->calculateColorCode($pVal, $rgbPositiveStartPoint, $rgbPositiveEndPoint);
                }
            } else {
                $val = round($val*-1);
                $maxToMin = $negativeMaxVal;
                $minToMax = $negativeMinVal;

                $negativeMinVal = abs($maxToMin);
                $negativeMaxVal = abs($minToMax);
                
                $pVal = $this->calculateValPerRetio($val, $negativeMinVal, $negativeMaxVal);
                if ($val == $negativeMaxVal) {
                    $colorCode = $this->fromRGBtoHex($rgbNegativeStartPoint['r'],$rgbNegativeStartPoint['g'],$rgbNegativeStartPoint['b']);
                }else if ($val == $negativeMinVal) {
                    $colorCode = $this->fromRGBtoHex($rgbNegativeEndPoint['r'],$rgbNegativeEndPoint['g'],$rgbNegativeEndPoint['b']);
                }else  if ($pVal == 0) {
                    $colorCode = $this->fromRGBtoHex($rgbNegativeStartPoint['r'],$rgbNegativeStartPoint['g'],$rgbNegativeStartPoint['b']);
                }else{
                    $colorCode = $this->calculateColorCode($pVal, $rgbNegativeStartPoint, $rgbNegativeEndPoint);
                }
            }
            return $colorCode;
        }

        public function fromRGBtoHex($R, $G, $B) {
            $R=dechex($R);
            if (strlen($R)<2)
            $R='0'.$R;
    ...
    /*[END] DYNAMIC COLORCODE GENERATION LOGIC*/
}
?>