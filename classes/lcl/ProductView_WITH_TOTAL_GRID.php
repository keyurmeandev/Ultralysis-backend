...
class ProductView extends config\UlConfig {
    ...
    function prepareGridData() {
        ...
        $tempResult = $tempTotalResult = array();
        if(isset($result) && !empty($result))
        {
            ...
            /*[START] GETTING THE TOTAL GRID DATA*/
                $this->settingVars->tableUsedForQuery = $this->measureFields = array();
                $this->measureFields[] = $this->skuID;
                $this->measureFields[] = $this->skuName;
                $this->measureFields[] = $this->distributionID;
                ...
                if(isset($result) && !empty($result)) {
                    ...
                    $share     = $total != 0 ? (($data[$havingTYValue] / $total) * 100) : 0;
                    $shareLY   = $totalSALESLY != 0 ? (($data[$havingLYValue] / $totalSALESLY) * 100) : 0;
                    $cros      = $data['DIS'] > 0 ? (($data[$havingTYValue] / $data['DIS']) / $week) : 0;
                    $crosLY    = $data['DISLY'] > 0 ? (($data[$havingLYValue] / $data['DISLY']) / $week) : 0;
                    $uros      = $data['DIS'] > 0 ? (($data['VOLUMETY'] / $data['DIS']) / $week) : 0;
                    $urosLY    = $data['DISLY'] > 0 ? (($data['VOLUMELY'] / $data['DISLY']) / $week) : 0;
                    ...
                    if ($data[$havingTYValue] > 0) {
                        ...
                        $tempTotalResult[] = $temp;
                    }
                }
            /*[END] GETTING THE TOTAL GRID DATA*/
        }
        $this->jsonOutput["gridValue"] = $tempResult;
        $this->jsonOutput["gridTotalValue"] = $tempTotalResult;
    }
