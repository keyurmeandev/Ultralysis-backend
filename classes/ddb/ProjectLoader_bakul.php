    ...
    *****/
        ...
        foreach($this->settingVars->filterPages as $mainKey => $filterPage)
        {
			if($filterPage['isDynamic'])
			{
				...
				foreach($filterPage['tabsConfiguration'] as $key => $tabsConfiguration)
				{
                    ...
                    $this->settingVars->filterPages[$mainKey]['tabsConfiguration'][$key]['breakdown'] = $this->hasBreakdown($tabsConfiguration['data'], $this->settingVars->filterPagesReplication[$mainKey]['config']);
				}
    ...
    private function setBreakdownList(){
        ...
        foreach ($breakdownData as $key => $value) {
            $this->breakdownList[$value['breakdownID']] = $value;
        }
    ...
    public function hasBreakdown($tableField, $config){
        ...
        return isset($this->breakdownData[$tableName][$dbColumn]) ? 1 : 0;
