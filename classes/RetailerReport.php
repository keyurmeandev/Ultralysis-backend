...
class RetailerReport extends config\UlConfig {
    /*     * ***
    ...
    public function go($settingVars) {
        ...
        if ($this->settingVars->isDynamicPage) {
            ...
            $this->brandFieldList = $this->getPageConfiguration('brand_list_field', $this->settingVars->pageID);
            if(!empty($this->brandFieldList)){
                $this->brandField = $this->brandFieldList[0];
            }

            if(isset($_REQUEST['accountID']) && !empty($_REQUEST['accountID'])){
                $this->brandField = $_REQUEST['accountID'];
            }
            ...
            $fieldArr[] = $this->supplierField;
            //$fieldArr[] = $this->brandField;
            if(!empty($this->mainFilterField))
                $fieldArr[] = $this->mainFilterField;
            $fieldArr[] = $this->supplierOrderByField;
            $fieldArr = array_merge($fieldArr,$this->brandFieldList);
            $buildDataFields = $fieldArr;
            
            $this->buildDataArray($buildDataFields);
            $this->buildPageArray();
            $brandListArr = [];
            if (isset($_REQUEST["fetchConfig"]) && !empty($_REQUEST["fetchConfig"]) && $_REQUEST["fetchConfig"] == 'true') {
                foreach ($this->brandFieldList as $key => $value) {
                    $brandFieldTmp = strtoupper($this->dbColumnsArray[$value]);
                    $brandListArr[] = ['data'  => $value, 'label' => $this->settingVars->dataArray[$brandFieldTmp]['NAME_CSV']];
                }
                $this->jsonOutput['brandFieldList'] = $brandListArr;
                
                $showWeekSelection = $this->getPageConfiguration('show_week_selection', $this->settingVars->pageID)[0];
                if(!empty($showWeekSelection))
                    $this->jsonOutput['showWeekSelection'] = $showWeekSelection;
                else
                    $this->jsonOutput['showWeekSelection'] = false;
            }
        }
    ...
    function genarateRetailerReport()
    {
        ...
        if(isset($this->settingVars->retailerReportTotalField) && !empty($this->settingVars->retailerReportTotalField))
            $selectPart[] = "MAX(".$this->settingVars->retailerReportTotalField.") as TOTAL ";
        ...
        $measureArr = $measureSelectionArr = $arrAliase = $havingPart = array();
        $timeWhereCluase = '';
            ...
            foreach ($this->timeArray as $timekey => $timeval) {
                if(is_array($this->settingVars->pageArray["MEASURE_SELECTION_LIST"]) && !empty($this->settingVars->pageArray["MEASURE_SELECTION_LIST"])){
                    ...
                    if ($timekey == '52') {
                        ...
                        $havingPart[] = $timeval.'_TY_'.$measure['ALIASE']." <> 0 OR ".$timeval.'_LY_'.$measure['ALIASE']." <> 0";
                    }
                }
            }
        ...
        //echo $query;exit;
        // echo '/usr/bin/perl '.dirname(__FILE__).'/../batch/RetailerReport.pl "'.$query.'"';
        // exit();
        // $result = shell_exec('/usr/bin/perl '.dirname(__FILE__).'/../batch/RetailerReport.pl "'.$query.'"');
        ...
        /*[START] SET PREPARED DATA INTO THE STATIC REDIS CACHE*/
            ...
            $this->redisCache->requestHash = $this->redisCache->prepareQueryHash('retailerReportCsvDataHashKey');
            $this->redisCache->setDataForStaticHash($reportData);
            $csvDataFilePath = $this->redisCache->requestHash;
        /*[END] SET PREPARED DATA INTO THE STATIC REDIS CACHE*/
        ...
        $maxMydate = (isset($this->settingVars->maxYearWeekCombination[2]) && $this->settingVars->maxYearWeekCombination[2] != "") ? date_format(date_create($this->settingVars->maxYearWeekCombination[2]), 'd F Y') : "";
        ...
        $this->supplierFieldName = (!empty($this->supplierFieldName)) ? ucfirst(strtolower($this->supplierFieldName)) : 'Market';
        $this->brandFieldName    = (!empty($this->brandFieldName)) ? ucfirst(strtolower($this->brandFieldName)) : 'Category';

        /*echo '/usr/bin/perl '.dirname(__FILE__).'/../batch/RetailerReport.pl "'.$filePath.'" "'.$csvDataFilePath.'" "'.$maxMydate.'" "'.$brands.'" "'.$supplierList.'" "'.$topFilterValue.'" "'.$timeFilterSettings.'" "'.$skipRankCnt.'" "'.$projectID.'" "'.$RedisServer.'" "'.$RedisPassword.'" "'.$this->supplierFieldName.'" "'.$this->brandFieldName.'"';*/

        $result = shell_exec('/usr/bin/perl '.dirname(__FILE__).'/../batch/RetailerReport.pl "'.$filePath.'" "'.$csvDataFilePath.'" "'.$maxMydate.'" "'.$brands.'" "'.$supplierList.'" "'.$topFilterValue.'" "'.$timeFilterSettings.'" "'.$skipRankCnt.'" "'.$projectID.'" "'.$RedisServer.'" "'.$RedisPassword.'" "'.$this->supplierFieldName.'" "'.$this->brandFieldName.'"');
    ...
    public function buildPageArray() {
        ...
        $this->supplierFieldName = $this->settingVars->dataArray[$supplierField]['NAME_CSV'];
        $this->brandFieldName = $this->settingVars->dataArray[$brandField]['NAME_CSV'];
