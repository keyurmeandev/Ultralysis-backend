...
class RetailerReport extends config\UlConfig {
    ...
    function genarateRetailerReport()
    {
        ...
        $brands = implode("##", $this->allBrandList);
        $supplierList = implode("##", $supplierList);
        $topFilterValue = implode("##", $topFilterValue);
        $timeFilterSettings = implode("##", $this->timeFilterSettings);

        $result = shell_exec("/usr/bin/perl /home/dev/public_html/testsh/writeExcel.pl '".$filePath."' '".$csvDataFilePath."' '".$maxMydate."' '".$brands."' '".$supplierList."' '".$topFilterValue."' '".$timeFilterSettings."'");
    ...
    public function generateRetailerReportCsvDataFile($reportData, $skipRank)
    {
        ...
        $this->allBrandList = array_values(array_unique(array_column($csvArray, 2)));
