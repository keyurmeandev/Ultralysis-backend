...
class timeFilter
{
    ...
    {
        self::$tyWeekRange  = " (". $settingVars->dateField ." BETWEEN '". self::$tyFromDate ."' AND '". self::$tyToDate ."') ";
        self::$lyWeekRange = "";
        
        if(isset(self::$lyFromDate) && !empty(self::$lyFromDate) && isset(self::$lyToDate) && !empty(self::$lyToDate))
            self::$lyWeekRange = " (". $settingVars->dateField ." BETWEEN '". self::$lyFromDate ."' AND '". self::$lyToDate ."') ";
        else
            self::$lyWeekRange = "";
    }
    ...
    *****/
    public static function getYearWeekWithinRange($from,$to,$settingVars,$hasTYLYRange = false){
        $queryVars  = projectsettings\settingsGateway::getInstance();
        $redisCache = new utils\RedisCache($queryVars);
        $yearWeekArray = array(); 
    ...
    ****/
    ...
    public static function getLatest_n_dates($s = '0', $n, $settingVars, $addLy = false) {
        $queryVars = projectsettings\settingsGateway::getInstance();
        $redisCache = new utils\RedisCache($queryVars);
        $mydateSelect = $settingVars->getMydateSelect($settingVars->dateField);
        $query = "SELECT " . $settingVars->yearperiod . " AS YEAR" .
        ...
        if(is_array($result) && count($result)>0){
            ...
            foreach ($result as $ky => $value) {
                array_push($daysArray, $value[0] . $value[1]);
                $LY = ($value[0] - 1);
                array_push($daysArrayLY, $LY . $value[1]);
            }
        }
        if($addLy){
