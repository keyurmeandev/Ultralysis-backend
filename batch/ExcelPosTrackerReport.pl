...
# ARGV[9] => Logo
# ARGV[10] => project ID
# ARGV[11] => Redis connection server
# ARGV[12] => Redis connection password

my $sql = $ARGV[1];
my $projectID      = $ARGV[10];
my $redisServer    = $ARGV[11];
my $redisPassword  = $ARGV[12];

my $redis = Redis->new( server => $redisServer, debug => 0, password => $redisPassword );
$redis->select($projectID);
my $pmst = $redis->get($sql);
my $queryResult = JSON->new->utf8->decode($pmst);
