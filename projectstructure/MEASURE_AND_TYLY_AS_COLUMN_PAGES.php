...
class MEASURE_AND_TYLY_AS_COLUMN_PAGES
{
	public function itemRankingReportTopBoxesLogic($settingVars, $queryVars, $storeIDField, $marketIds = array(), $performanceBoxSettings = array()) 
	{
		...
		if (is_array($marketIds) && !empty($marketIds)) {
			foreach ($marketIds as $index => $currentMarketID) {
	            foreach ($performanceBoxSettings as $boxSetting) {
	            	filters\timeFilter::getSliceTimeframe($settingVars, $boxSetting['timeFrame']);

	                $options['tyLyRange']['SALES_'.$boxSetting['alias'].'_TY_'.$index] = " ".$storeIDField." ='" . $currentMarketID . "' AND ".filters\timeFilter::$tyWeekRange." ";
	                $options['tyLyFieldPart']['SALES_'.$boxSetting['alias'].'_TY_'.$index] = trim(filters\timeFilter::$tyTimeframeRange);
	                $options['tyLyRange']['SALES_'.$boxSetting['alias'].'_LY_'.$index] = " ".$storeIDField." ='" . $currentMarketID . "' AND ".filters\timeFilter::$lyWeekRange." ";
	                $options['tyLyFieldPart']['SALES_'.$boxSetting['alias'].'_LY_'.$index] = trim(filters\timeFilter::$lyTimeframeRange);
	            }
		...
		return array('havingPart' => $havingPart, 'measureSelect' => $measureSelect);
	}
	...
	public function itemRankingReportMarketGridLogic($settingVars, $queryVars, $storeIDField, $marketIds = array()) 
	{
		...
		if (is_array($marketIds) && !empty($marketIds)) {
			foreach ($_REQUEST['marketID'] as $marketIndex => $currentMarketID) {
				...
				if (!empty($currentMarketID) && $currentMarketID != "") {
					...
					$selectPartForTotal['tyLyFieldPart']['TOTAL_'.$marketIndex] = trim(filters\timeFilter::$tyTimeframeRange);

					$options['tyLyRange']['SALES_TY_'.$marketIndex] =  " ".$storeIDField." ='" . $currentMarketID . "' ";
					$options['tyLyFieldPart']['SALES_TY_'.$marketIndex] = trim(filters\timeFilter::$tyTimeframeRange);

					$options['tyLyRange']['SALES_LY_'.$marketIndex] =  " ".$storeIDField." ='" . $currentMarketID . "' ";
					$options['tyLyFieldPart']['SALES_LY_'.$marketIndex] = trim(filters\timeFilter::$lyTimeframeRange);
		...
		return array('havingPart' => $havingPart, 'measureSelect' => $measureSelect, 'measureSelectForTotal' => $measureSelectForTotal);
	}
    
    public function rangeEfficiencyCustomSelectPart($settingVars, $countAccount = "")
    {
        $result = array();
        $result['customSelectPart'] = "SUM(".$settingVars->ProjectDist."_".filters\timeFilter::$tyTimeframeRange . ") AS STORES ";
        return $result;
    }

    public function extraWhereClause($settingVars, &$measureFields)
    {
        if( isset($settingVars->controlFlagField) && !empty($settingVars->controlFlagField) )
        {
            $measureFields[] = $settingVars->controlFlagField;
            return " AND ".$settingVars->controlFlagField." = 0";
        }
        else
            return "";
    }
    ...
    public function extraWhereClauseForTopBoxes($settingVars, &$measureFields)
    {
        ...
        {
            $measureFields[] = $settingVars->controlFlagField;
            return " AND ".$settingVars->controlFlagField." = 1";
        }
        else
            return "";
    }
